...
import { DeepLink, deepLinkPrefixes, RegexConst } from '../constants';
...
export function isDeepLinkType(
  ...
  const isMatchParams: boolean = _.has(params, type);
...
function convertUrlToDeepLink(
  ...
  if (isDeepLinkType(routeName, params, DeepLink.MagicLink)) {
    return `stickersmashexpoboiler://auth/signIn/${encodeURIComponent(url)}/${params?.tenantId}/${params?.email}`;
  } else if (isDeepLinkType(routeName, params, DeepLink.ForgotPassword)) {
    return `stickersmashexpoboiler://auth/createNewPassword/${params?.oobCode}/${params?.tenantId}/${params?.email}`;
  }
  return undefined;
}
...
function getUrlParams(branchUrl: string): Record<string, string> {
  ...
  let regex: RegExp = RegexConst.urlParams;
  let match;
...
export function checkAndGetParams(nonBranchUrl: string): CheckAndGetParamsReturnType | undefined {
  ...
  try {
    ...
    if (newParams.continueUrl && !_.isEmpty(newParams.continueUrl)) {
